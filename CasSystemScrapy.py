...
def getContent(url, header=None, datas=None, cookie=None):
    ...
    if header == None:
        _header = {
            'User-Agent':
            'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36',  #模拟登陆的浏览器
            'Accept':
    ...
    while True:
        try:
            rep = requests.request("POST",
                                   ...
                                   data=datas,
                                   timeout=timeout,
                                   cookies=cookie)  #请求url地址，获得返回 response 信息
            rep.encoding = 'utf-8'
            break
...
def getpagecontent(pageno, cookie):
    ...
    headers = {
        ...
        'Origin': 'http://10.9.140.98',
        'Referer':
        'http://10.9.140.98/workflow_skc/apps/index.cfm?fuseaction=inquiryall',
        'Upgrade-Insecure-Requests': '1',
        'cookie': cookie
    }
    try:
    ...
    try:
        ...
        for tr in tr_arr:
            #//查询所有td
            tds = tr.find_all('td')
            if len(tds) != 11 or tds[0].text.strip(
            ) == '目录' or tds[0].text.strip() == '管理序号':
                continue
            temp = []
...
def insert_CasTravel(travelobject):
    ...
    try:
        ...
        if len(detiallist) > 0 and len(detiallist) == 1:
            ...
            castravel.append((time.strftime("%Y-%m-%d %H:%M:%S",
                                            time.localtime())))  #CreateDate
            castravel.append((time.strftime("%Y-%m-%d %H:%M:%S",
                                            time.localtime())))  #ModifyDate
        ...
        log.error(
            f"将派车单号{travelobject['applyno']}新增转化为sql出错：Unexpected Error: {e}")
...
def create_apply_sql(travelobject, detiallist):
    ...
    castravel.append(castraveldid)  #DID
    castravel.append(
        (travelobject['applyno']) + '-' + (detiallist['usedate']))  #派车单号
    castravel.append(travelobject['applyno'])  # bwf管理序号
    castravel.append(1)  # 1 待派车 状态
    castravel.append(travelobject['applytype'])  #申请状态类型
    castravel.append(travelobject['bwfstatus'])  #bwf状态
...
def get_apply_data(pageno, applyids, cookie, applyno, bwfstatus):
    ...
    try:
        ...
        table2 = inputs[5]
        applytpye = table2.xpath(
            "./tr[1]/td/table/tr/td[2]/span/textarea")[0].text  #申请用车类型
        AcutalUser = table2.xpath(
            "./tr[2]/td/table/tr/td[4]/span/input")[0].get(
                'value', default=None)  #实际使用者
        Peoples = table2.xpath("./tr[2]/td/table/tr/td[2]/span/input")[0].get(
            'value', default=None)  #实际人数
        InSideLine = table2.xpath(
            "./tr[2]/td/table/tr/td[6]/span/input")[0].get('value',
                                                           default=None)  #内线
        reason = table2.xpath("./tr[1]/td/table/tr/td[4]/span/input")[0].get(
            'value', default=None)  #变更、取消理由
        usereason = table2.xpath(
            "./tr[6]/td/table/tr/td[2]/span/input")[0].get('value',
                                                           default=None)  #用车理由
        DetailedAddres = table2.xpath(
            "./tr[7]/td/table/tr/td[2]/span/input")[0].get(
                'value', default=None)  #行程详细地址

        phone1 = table2.xpath("./tr[8]/td/table/tr/td[2]/span/input")[0].get(
            'value', default=None)  #联系人手机
        passenger1 = table2.xpath(
            "./tr[8]/td/table/tr/td[4]/span/input")[0].get('value',
                                                           default=None)  #联系人1

        phone2 = table2.xpath("./tr[9]/td/table/tr/td[2]/span/input")[0].get(
            'value', default=None)  #联系人手机
        passenger2 = table2.xpath(
            "./tr[9]/td/table/tr/td[4]/span/input")[0].get('value',
                                                           default=None)  #联系人2

        phone3 = table2.xpath("./tr[10]/td/table/tr/td[2]/span/input")[0].get(
            'value', default=None)  #联系人手机
        passenger3 = table2.xpath(
            "./tr[10]/td/table/tr/td[4]/span/input")[0].get(
                'value', default=None)  #联系人3

        phone4 = table2.xpath("./tr[11]/td/table/tr/td[2]/span/input")[0].get(
            'value', default=None)  #联系人手机
        passenger4 = table2.xpath(
            "./tr[11]/td/table/tr/td[4]/span/input")[0].get(
                'value', default=None)  #联系人4

        phone5 = table2.xpath("./tr[12]/td/table/tr/td[2]/span/input")[0].get(
            'value', default=None)  #联系人手机
        passenger5 = table2.xpath(
            "./tr[12]/td/table/tr/td[4]/span/input")[0].get(
                'value', default=None)  #联系人5

        ShareCore = table2.xpath(
            "./tr[14]/td/table/tr/td[2]/span/input")[0].get(
                'value', default=None)  #经费代码
        CostDepartment = table2.xpath(
            "./tr[13]/td/table/tr/td[4]/span/input")[0].get('value',
                                                            default=None)  #部门
        applyperson = table2.xpath(
            "./tr[21]/td/table/tr[5]/td[2]/span")[0].text  #申请人
        applydate = table2.xpath(
            "./tr[21]/td/table/tr[6]/td[2]/span")[0].text  #申请日期
        createtime = (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
        Company = table2.xpath(
            "./tr[13]/td/table/tr/td[2]/span/textarea")[0].text  #公司
        ...
        for item in trs:
            ...
            if len(tds) > 0:
                ...
                usedate = tds[1].xpath("./span/input")[0].get(
                    'value', default=None)  #用车日期
                start = tds[2].xpath("./span/input")[0].get('value',
                                                            default=None)  #出发地
                travel1 = tds[3].xpath("./span/input")[0].get(
                    'value', default=None)  #目的地1
                travel2 = tds[4].xpath("./span/input")[0].get(
                    'value', default=None)  #目的地2
                travel3 = tds[5].xpath("./span/input")[0].get(
                    'value', default=None)  #目的地3
                travel4 = tds[6].xpath("./span/input")[0].get(
                    'value', default=None)  #目的地4
                travel5 = tds[7].xpath("./span/input")[0].get(
                    'value', default=None)  #目的地5
                travel6 = tds[8].xpath("./span/input")[0].get(
                    'value', default=None)  #目的地6
                OstartTime = tds[9].xpath("./span/input")[0].get(
                    'value', default=None)  #出发日期
                ReturnTime = tds[10].xpath("./span/input")[0].get(
                    'value', default=None)  #返回日期
                FlightNo = tds[11].xpath("./span/input")[0].get(
                    'value', default=None)  #航班号
                FlightTime = tds[12].xpath("./span/input")[0].get(
                    'value', default=None)  #起飞到达日期
                detials = {
                    'usedate': usedate,
                    'travel1': travel1,
                    'travel2': travel2,
