...
def auto_key_and_click():
    # 自动按键
    win32api.keybd_event(win32con.VK_SPACE, 0, 0, 0)  # 模拟按下空格键
    time.sleep(1)  # 按键持续时间
    win32api.keybd_event(win32con.VK_SPACE, 0, win32con.KEYEVENTF_KEYUP, 0)  # 松开空格键

    # 自动鼠标点击
    x = 500  # 鼠标点击的横坐标
    y = 500  # 鼠标点击的纵坐标
    win32api.SetCursorPos((x, y))  # 设置鼠标位置
    win32api.mouse_event(win32con.MOUSEEVENTF_LEFTDOWN, x, y, 0, 0)  # 模拟鼠标左键按下
    time.sleep(1)  # 鼠标点击持续时间
    win32api.mouse_event(win32con.MOUSEEVENTF_LEFTUP, x, y, 0, 0)  # 模拟鼠标左键松开
