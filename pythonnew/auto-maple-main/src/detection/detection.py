...
# 过滤掉在 HSV 颜色空间中不在橙色到绿色之间的所有颜色，以消除箭头周围的某些噪音。
def filter_color(image):
    """
    Filters out all colors not between orange and green on the HSV scale,
    which eliminates some noise around the arrows.
...
def merge_detection(model, image):
    ...
    converter = {'up': 'right', 'down': 'left'}  # 用于“旋转推断”的转换器
    classes = []
    ...
    if len(boxes) == 4:      # 仅在正确检测到箭头时运行进一步的推断
